...
exports.list = async (req, res) => {
  ...
  data = await db.excute(
    `select post.no as no, post.title as title, post.body as body, post.createdAt as createdAt, category.no as cate, category.name as category from post inner join category on category.no=post.category ${
      category > 0 ? ` where post.category=${category}` : ''
